...
const addFeature = () => {
  const feature = new Feature({
    geometry: new LineString([
      [116.4074, 39.9042], // 北京市中心经纬度
      [118.7784, 32.0647], // 南京新街口经纬度
    ]),
    name: "Beijing",
  });
  ...
  const layer = new VectorLayer({
    ...
    style: new Style({
      ...
      text: new Text({
        ...
        placement: "line", // 关键点：沿线显示
        overflow: true, // 允许文字超出线段范围显示
      }),
    }),
  });
  // 沿线生成标注点（示例取中点）
  ...
  const labelPoint = new Feature({
    ...
    type: "lineLabel",
    labelText: "主要路段", // 自定义文本
  });

  // 设置文字样式
  const labelStyle = new Style({
    text: new Text({
      text: labelPoint.get("labelText"),
      font: "bold 14px 微软雅黑",
      fill: new Fill({ color: "#000" }),
      stroke: new Stroke({ color: "#fff", width: 3 }),
      offsetY: -15, // 垂直偏移避让线
      placement: "line", // 沿路径方向（OpenLayers 6+支持）
      textAlign: "center",
    }),
  });
  labelPoint.setStyle(labelStyle);

  // 添加至图层
  layer.getSource().addFeature(labelPoint);
  map.addLayer(layer);
