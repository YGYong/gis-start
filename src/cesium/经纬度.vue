...
onMounted(async () => {
  ...
  // 坐标系转换
  // 1. WGS84 转 Cartesian3
  const tiananmenCartesian = Cesium.Cartesian3.fromDegrees(
    116.3975, // 经度（度）
    39.9075, // 纬度（度）
    50 // 高度（米）
  );
  console.log("WGS84 转 Cartesian3:", tiananmenCartesian);

  // 2. Cartesian3 转 WGS84
  // 笛卡尔坐标转弧度坐标
  ...
  console.log("经纬度 ↔ 弧度:" + lonRad, latRad, lonDeg + "°", latDeg + "°");
  ...
  // WGS84 转 Web 墨卡托
  const cartographic1 = Cesium.Cartographic.fromDegrees(116.3975, 39.9075);
  const projected = projection.project(cartographic1);
